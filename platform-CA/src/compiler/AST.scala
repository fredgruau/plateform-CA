 ...
 * use mutableDag, and not simply Dag, in order to be able to do substitution.
 */

abstract class AST[+T]()(implicit m: repr[T]) extends Dag[AST[_]] with Named {
  val mym = m //if type of mym is set to repr[_] this allow covariance even if repr is not covariant
  ...
  /**@a is true where there is a bug associated to the field "this" */
  def bugif(a: ASTLt[_ <: Locus, B]) = { sysInstr ::= UsrInstr.bugif(a).asInstanceOf[UsrInstr[AST[_]]] }
  /** @a allows to visualize "this" with a minimalistic coloring load on the screen*/
  def render(a: ASTL.ASTLg) { sysInstr ::= UsrInstr.display(a).asInstanceOf[UsrInstr[AST[_]]] }

  def checkName() = { if (name == null) name = "_aux" + AST.getCompteur; this }
  /** Builds the set of affected symbols which are read. */
  def reads: immutable.HashSet[String] = this match {
    case Read(s) => HashSet(s)
    case _       => neighbor.map(e => e.reads).foldLeft(HashSet.empty[String])((x, y) => x.union(y))
  }
  ...
  def symbols: immutable.HashSet[String] = this match {
    ...
    case _                   => neighbor.map(e => e.symbols).foldLeft(HashSet.empty[String])((x, y) => x.union(y))
  }
   ...
   * @usedTwice  dags which are used twice, or which need to be affected for some other reason.
   * @repr: representant of the equivalence class with respect to equal on case class hierarchy
   * @replaced: map encoding a substitution.
  ...
  def deDag(usedTwice: immutable.HashSet[AST[_]], repr: Map[AST[_], AST[_]], replaced: Map[AST[_], AST[_]]): AST[T] = {
    if (usedTwice.contains(this)) new Read[T](repr(this).name)(mym.asInstanceOf[repr[T]])
      ...
      case Param(s) => throw new RuntimeException("bordel de merde"); new Read[T](repr(this).name)(mym.asInstanceOf[repr[T]])
  ...
  def nbit(cur: ProgData1[_], nbitLB: AstField[Int], tSymb: TabSymb[InfoNbit[_]], newFuns: TabSymb[ProgData2]): AST[T] = {
    var newthis = this.propagate((d: AST[T]) => d.nbit(cur, nbitLB, tSymb, newFuns))
  ...
  def propagate(id1: bij2[_]): AST[T] = {
    ...
    val newD = this.asInstanceOf[AST[_]] match {
      case Read(s)                 => this //throw  new RuntimeException("Deja dedagifié!" )
      case e: EmptyBag[_]          => e
      ...
      case e @ Call1(f, a)         => e.copy(arg = id2(a))(e.mym)
      case e @ Call2(f, a, a2)     => e.copy(arg = id2(a), arg2 = id2(a2))(e.mym)
      case e @ Call3(f, a, a2, a3) => e.copy(arg = id2(a), arg2 = id2(a2), arg3 = id2(a3))(e.mym)
    }; newD.setName(this.name); newD.asInstanceOf[AST[T]]
  }
  def concatElt: Boolean = this match {
    case a: EmptyBag[_] => true
    case _              => false
  }
...
object AST {
  ...
  trait EmptyBag[T <: Dag[T]] extends Dag[T] { def neighbor: List[T] = List.empty; }
  trait Singleton[T <: Dag[T]] extends Dag[T] { def arg: T; def neighbor: List[T] = List(arg) }
  trait Doubleton[T <: Dag[T]] extends Dag[T] { def arg: T; def arg2: T; def neighbor: List[T] = List(arg, arg2) }
  trait Tripleton[T <: Dag[T]] extends Dag[T] { def arg: T; def arg2: T; def arg3: T; def neighbor: List[T] = List(arg, arg2, arg3) }
  trait Neton[T <: Dag[T]] extends Dag[T] { def args: List[T]; def neighbor: List[T] = args  }

  private var nameCompteur = 0;
  def getCompteur: Int = { nameCompteur += 1; nameCompteur }
  def checkName2() = "_aux" + AST.getCompteur;
  type bij2[U] = AST[U] => AST[U]
  case class Coons[Thead, Ttail](val arg: AST[Thead], val arg2: AST[Ttail])(implicit n: repr[Tuple2[Thead, Ttail]]) extends AST[Tuple2[Thead, Ttail]] with Doubleton[AST[_]] //{ def setArg(a: AST[_]) = arg = a.asInstanceOf[AST[Thead]]; def setArg2(a: AST[_]) = arg2 = a.asInstanceOf[AST[Ttail]] }
  case class Heead[Thead](val arg: AST[Tuple2[Thead, _]])(implicit n: repr[Thead]) extends AST[Thead] with Singleton[AST[_]] //{ def setArg(a: AST[_]) = arg = a.asInstanceOf[AST[Tuple2[Thead, _]]]; }
  case class Taail[Ttail](val arg: AST[Tuple2[_, Ttail]])(implicit n: repr[Ttail]) extends AST[Ttail] with Singleton[AST[_]] //{ def setArg(a: AST[_]) = arg = a.asInstanceOf[AST[Tuple2[_, Ttail]]]; }
  ...
  case class Fundef0[+To1](override val namef: String, val arg: AST[To1]) extends Fundef[To1](namef, arg)
  case class Fundef1[+Ti1, +To1](override val namef: String, val arg: AST[To1], val p1: Param[Ti1]) extends Fundef[To1](namef, arg, p1)
  case class Fundef2[+Ti1, +Ti2, +To1](override val namef: String, val arg: AST[To1], val p1: Param[Ti1], val p2: Param[Ti2]) extends Fundef[To1](namef, arg, p1, p2)
  case class Fundef3[+Ti1, +Ti2, +Ti3, +To1](override val namef: String, val arg: AST[To1], val p1: Param[Ti1],
                                             val p2: Param[Ti2], val p3: Param[Ti3]) extends Fundef[To1](namef, arg, p1, p2, p3)
  //on peut pas utiliser fundefn, car faudrait savoir a l'avance le nombre de paramétres, pour maj l'environnement.
  case class Fundefn[Ti1, To1](override val namef: String, val arg: AST[To1], val pn: Param[Ti1]*)(implicit n: repr[To1])
    extends Fundef[To1](namef, arg, pn: _*)
  case class Param[+T](val nameP: String)(implicit n: repr[T]) extends AST[T] with EmptyBag[AST[_]] { name = nameP; }
  ...
  case class Call1[Ti1, To1](override val f: Fundef1[Ti1, To1], val arg: AST[_ <: Ti1])(implicit n: repr[To1])
    extends Call[To1](f, arg) with Singleton[AST[_]]
  case class Call2[Ti1, Ti2, To1](override val f: Fundef2[Ti1, Ti2, To1], val arg: AST[_ <: Ti1], val arg2: AST[_ <: Ti2])(implicit n: repr[To1])
    extends Call[To1](f, arg, arg2) with Doubleton[AST[_]]
  case class Call3[Ti1, Ti2, Ti3, To1](override val f: Fundef3[Ti1, Ti2, Ti3, To1], val arg: AST[_ <: Ti1], val arg2: AST[_ <: Ti2], val arg3: AST[_ <: Ti3])(implicit n: repr[To1])
  ...
  case class Delayed[T](_arg: () => AST[T])(implicit m: repr[T]) extends AST[T]() with Singleton[AST[_]] { lazy val arg = { /* _arg().user+=this;*/ _arg() } }
