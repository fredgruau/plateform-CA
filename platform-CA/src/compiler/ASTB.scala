...
/** node of Abstract Syntax Tree corresponding to an arithmetic field   */
...
/** The direction left means we loop from the least significant bits towards the msb, and direction right is the other way round. */
...
object ASTB { //
  ...
  val carry: Fundef3R[B] = {
   ...
   val add: Fundef2R[I] = {val (x,y)  = (p[I]("x"), p[I]("y")); Fundef2("add", Xor(Xor(x, y), Scan2(x, y, carry, False[B], Left(), true)), x, y) }
  ...
  val minSI: Fundef2R[SI] = {
   val (xsi,ysi)  = (p[SI]("xsi"), p[SI]("ysi"));    Fundef2("minSI", Mapp2(xsi, ysi, xorB), xsi, ysi)  } //TODO a faire correct en utilisant gt.
  val minUI: Fundef2R[UI] = {val (xui,yui)  = (p[UI]("xui"), p[UI]("yui"));    Fundef2("minUI", Mapp2(xui, yui, xorB), xui, yui)
  ...
  def elt(i: Int): Fundef1[I, B] = { val x = p[I]("x"); Fundef1("elt", Elt[I](i, x), x) }
   def extend(i: Int): Fundef1[I, I] = { val x = p[I]("x"); Fundef1("extend", Extend[I](i, x), x) }
