 ...
 */
sealed abstract class ASTL[L <: Locus, R <: Ring]()(implicit m: repr[Tuple2[L, R]]) extends ASTLt[L, R] {
  override def toString: String =
    this.asInstanceOf[ASTL[_, _]] match {
      //  case Layer(s, _)                 => "Layer " + this.name + ":" + locus.toString.charAt(0) + "-" + ring.toString.charAt(0)
      case l: Layer[_, _]              => "Layer " + this.name + ":" + locus.toString.charAt(0) + "-" + ring.toString.charAt(0)
      case Const(cte, _, _)            => "Const" + cte.toString + locus.toString.charAt(0) + "_" + ring.toString.substring(0, ring.toString.length() - 2);
      case Binop(op, arg1, arg2, _, _) => op.namef
      //  case Multop(op, args,_,_)      => op.toString
      case Unop(op, arg, _, _)         => op.namef
      case Redop(op, arg, _, _)        => "red" + op._1.namef
      case Clock(arg, dir, _, _)       => if (dir) "clock" else "anticlock"
      case e @ Broadcast(arg, _, _)    => "Broadcast" + ("" + (e.locus.asInstanceOf[T[_, _]] match { case T(x, y) => y })).charAt(0)
      case e @ Send(args)              => "Send" + ("" + (e.locus.asInstanceOf[T[_, _]] match { case T(x, y) => y })).charAt(0)
  ...
  def propagate(id: bij[L, R]): ASTL[L, R] = {
    ...
    val newD = this.asInstanceOf[ASTLg] match {
      ...
      case e @ Send(a)               => e.copy(args = a.map(id2(_)))(lpart(e.mym), rpart(e.mym))
      ...
      case e @ Clock(a, _, _, _)     => e.copy(arg = id2(a))
    ...
    val result = this match {
      ...
      case _ => { //in all the other cases, no change is done on the AST, only  nbitLB is updated.
        ...
        nbitLB += newthis -> (newthis.asInstanceOf[ASTL[_, _]] match {
          ...
          case Unop(op, arg, _, _) => ASTB.nBitR(nbitB + (op.p1 -> newNbit()), op.arg, nbitP)
          case Redop(_, _, _, _) | Clock(_, _, _, _) | Transfer(_, _, _) | Broadcast(_, _, _) | Sym(_, _, _, _) => newNbit()
          case Send(a) => nbitLB(newthis.asInstanceOf[Neton[AST[_]]].args.head)
  ...
  override def unfoldSimplic(m: Machine): ArrAst = {
    ...
    this.asInstanceOf[ASTLg] match {
      ...
      case Broadcast(a, _, _) => throw (new RuntimeException("Broadcast creates   a transfer type"))
      case Send(a)            => throw (new RuntimeException("Broadcast creates   a transfer type"))
      case Transfer(a, _, _)  => throw (new RuntimeException("Transfer creates   a transfer type"))
      ...
      case Clock(a, dir, _, _) => throw (new RuntimeException("Clock creates    a transfer type"))

      case Sym(a, _, _, _)     => throw (new RuntimeException("Sym creates  a transfer type"))
    }
  ...
  override def unfoldTransfer(m: Machine): ArrArrAst = {
    ...
    this.asInstanceOf[ASTLg] match {
      ...
      case Broadcast(a, _, _) => a.unfoldSimplic(m).map(Array.fill(l2)(_)).asInstanceOf[ArrArrAst]
      case Send(a) =>
      ...
      case Redop(op, a, _, _) => throw (new RuntimeException("Reduces create a simplicial type, not a transfer type"))
      case Clock(a, dir, _, _) =>
        val T(_, src) = a.locus
        val trigo = !(dir)
      ...
      case Sym(a, _, _, _) =>
        val T(s1, src) = a.locus; val atr = a.unfoldTransfer(m).map(rotR(_)); s1 match {
          case V() => throw (new RuntimeException("sym not defined on V in the general case"))
  ...
  def shouldAffect: Boolean = this match {
    ...
    case Binop(_, a1, a2, _, _) => a1.isInstanceOf[ASTL.Clock[_, _, _, _]] || a2.isInstanceOf[ASTL.Clock[_, _, _, _]] //that's an overkill, unnecessary introduced variables will have to be removed
    case _                      => false
  }
...
object ASTL {
  ...
  private[ASTL] final case class Transfer[S1 <: S, S2 <: S, R <: Ring](val arg: ASTLt[T[S1, S2], R], m: repr[T[S2, S1]], n: repr[R])
    extends ASTL[T[S2, S1], R]()(repr.nomLR(m, n)) with Singleton[AST[_]]
  private[ASTL] final case class Unop[L <: Locus, R1 <: Ring, R2 <: Ring](op: Fundef1[R1, R2], val arg: ASTLt[L, R1], m: repr[L], n: repr[R2])
    extends ASTL[L, R2]()(repr.nomLR(m, n)) with Singleton[AST[_]]
  private[ASTL] final case class Binop[L <: Locus, R1 <: Ring, R2 <: Ring, R3 <: Ring](op: Fundef2[R1, R2, R3], val arg: ASTLt[L, R1], val arg2: ASTLt[L, R2], m: repr[L], n: repr[R3])
    extends ASTL[L, R3]()(repr.nomLR(m, n)) with Doubleton[AST[_]]
  private[ASTL] final case class Redop[S1 <: S, S2 <: S, R <: Ring](op: redop[R], val arg: ASTLt[T[S1, S2], R], m: repr[S1], n: repr[R])
    extends ASTL[S1, R]()(repr.nomLR(m, n)) with Singleton[AST[_]]
  private[ASTL] final case class Clock[S1 <: S, S2 <: S, S3 <: S, R <: Ring](val arg: ASTLt[T[S1, S2], R], dir: Boolean, m: repr[T[S1, S3]], n: repr[R])
  ...
  abstract class Layer[L <: Locus, R <: Ring](val nbit: Int)(implicit m: repr[L], n: repr[R]) extends ASTL[L, R]() with EmptyBag[AST[_]] with Strate[L, R] {
    ...
    override def other = next :: super.other
    /** instructions also includes updating the layer by storing the next value.  */
    override def sysInstrs = UsrInstr.memorize(next).asInstanceOf[UsrInstr[AST[_]]] :: super.sysInstrs
  }
...
/*****************the wrapper*******************/
  ...
  def sym[S1 <: S, S2 <: S, S3 <: S, R <: Ring](arg: ASTLt[T[S2, S1], R])(implicit m: repr[T[S2, S3]], t: CentralSym[S1, S2, S3], n: repr[R]) = Sym(arg, m, t, n);
  ...
  def orScanRight[L <: Locus, R <: I](arg1: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) = Unop[L, R, R](halveB.asInstanceOf[Fundef1[R, R]], arg1, m, n)
  def gt[L <: Locus](arg1: ASTLt[L, SI])(implicit m: repr[L]) = Unop[L, SI, B](elt(-1).asInstanceOf[Fundef1[SI, B]], arg1, m, repr.nomB); //-1 will be taken modulo nbit.
  def notNull[L <: Locus, R <: I](arg1: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) = Unop[L, R, B](notNullB.asInstanceOf[Fundef1[R, B]], arg1, m, repr.nomB);
  /** Instead of casting boolean to integer,  we define a logical and taking an int and a  bool*/
  def andLB2R[L <: Locus, R <: I](arg1: ASTLt[L, B], arg2: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) = Binop[L, B, R, R](andLBtoR.asInstanceOf[Fundef2[B, R, R]], arg1, arg2, m, n);
  def elem[L <: Locus, R <: I](i: Int, arg: ASTLt[L, R])(implicit m: repr[L], n: repr[B]) = Unop[L, R, B](elt(i).asInstanceOf[Fundef1[R, B]], arg, m, n);
  ...
  def extend[L <: Locus, R <: Ring](i: Int, arg: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) = Unop[L, R, R](ASTBfun.extend(i).asInstanceOf[Fundef1[R, R]], arg, m, n);
  ...
  def orR[S1 <: S, S2 <: S, R <: Ring](arg: ASTLt[T[S1, S2], R])(implicit m: repr[S1], n: repr[R]) = Redop[S1, S2, R](orRedop[R], arg, m, n);
  def andR[S1 <: S, S2 <: S, R <: Ring](arg: ASTLt[T[S1, S2], R])(implicit m: repr[S1], n: repr[R]) = Redop[S1, S2, R](andRedop[R], arg, m, n);
  def xorR[S1 <: S, S2 <: S, R <: Ring](arg: ASTLt[T[S1, S2], R])(implicit m: repr[S1], n: repr[R]) = Redop[S1, S2, R](xorRedop[R], arg, m, n);

  def redOp2[S1 <: S, S2 <: S, S2new <: S, R <: Ring](op: redop[R], arg: ASTLt[T[S1, S2], R], m: repr[T[S1, S2new]], n: repr[R]) =
    Binop[T[S1, S2new], R, R, R](op._1, Clock[S1, S2, S2new, R](arg, true, m, n), Clock[S1, S2, S2new, R](arg, false, m, n), m, n)

  def xorR2[S1 <: S, S2 <: S, S2new <: S, R <: Ring](arg: ASTLt[T[S1, S2], R])(implicit m: repr[T[S1, S2new]], n: repr[R]) =
    redOp2[S1, S2, S2new, R](xorRedop[R], arg, m, n);
  ...
  def delayedL[L <: Locus, R <: Ring](_arg: => ASTLt[L, R])(implicit m: repr[Tuple2[L, R]]): ASTLt[L, R] = { lazy val delayed = _arg; new Delayed[Tuple2[L, R]](() => delayed) with ASTLt[L, R] }
  def cond[L <: Locus, R <: I](b: ASTLt[L, B], arg1: ASTLt[L, R], arg2: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) =
  ...
  def or[L <: Locus, R <: Ring](arg1: ASTLt[L, R], arg2: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) = (arg1.ring match {
  ...
  def and[L <: Locus, R <: Ring](arg1: ASTLt[L, R], arg2: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) = (arg1.ring match {
  ...
  def xor[L <: Locus, R <: Ring](arg1: ASTLt[L, R], arg2: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) = (arg1.ring match {
  ...
  def neg[L <: Locus, R <: Ring](arg: ASTLt[L, R])(implicit m: repr[L], n: repr[R]) = (arg.ring match {
