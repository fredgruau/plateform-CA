...
class Circuit extends Bags with BagLibrary{
...
//  var layers= new ListBuffer[Layer[_<:Locus,_<:Ring]]()  
 /** records all the AST nodes, in order to easily iterate on them */
   ...
   lazy val layers:List[ LayerAST[_<:Locus,_<:Ring]]=  nodes.toList.collect{ case l:LayerAST[_,_ ] =>l }
 ...
 /**Prints all the instructions */
   override def toString:String= "AST has "+ nodes.size + " nodes, grouped into " + instr.size + " instructions " + instr.map( _ .toString).foldLeft("")(_ + ",\n " + _)
}
